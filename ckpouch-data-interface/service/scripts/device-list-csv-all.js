...
const sUtils = {
     ...
     */
    streamCsvToResponse: async (ctx, res, filePath, offset, limit, columns, dateFormatted, isFirst, isEnd) => {
        const { log } = ctx;
        return new Promise((resolve, reject) => {
            ...
            const startLine = (offset || 0) + 1; // +1 because line 0 is header
            const endLine = startLine + (limit || Infinity);
...
module.exports = async (ctx, src, packet, listener) => {
    ...
    const op = 'POST /api/device/list/all -';
    const lhd = `[${src}:${tid}] ${op}`;
    log.info(`${lhd} >> start list all device. data [${JSON.stringify(packet.dt)}]`);
    ...
    if (!selectDeviceSyncHisInfo.succ) {
        log.error(`${lhd} << failed list all device. failed select device sync his. err [${selectDeviceSyncHisInfo.err}]`);
    ...
    if (selectDeviceSyncHisInfo.data.length <= 0) {
        log.warn(`${lhd} << success list all device with nothing. no data found`);
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            log.error(`${lhd} << failed list all device. file not found [${filePath}]`);
            return { result: 'FAIL', err: 'file not found' };
        }

        // add custom
        const itemCount2 = await sUtils.streamCsvToResponse(ctx, res, '/data/files/device-list/2026/04/03/device-list-20260403130400.csv', 0, 10000000, columns, dateFormatted, true, false);
        log.debug(`${lhd} work 2`);

        // Stream CSV directly to response
        const itemCount = await sUtils.streamCsvToResponse(ctx, res, filePath, 0, 10000000, columns, dateFormatted, false, true);
        log.debug(`${lhd} work 1`);

        // // Stream CSV directly to response
        // const itemCount = await sUtils.streamCsvToResponse(ctx, res, filePath, null, null, columns, dateFormatted, true, false);
        // log.debug(`${lhd} work 1`);

        // // add custom
        // const itemCount2 = await sUtils.streamCsvToResponse(ctx, res, '/data/files/device-list/2026/04/03/device-list-20260403130400.csv', null, null, columns, dateFormatted, false, true);
        // log.debug(`${lhd} work 2`);
        
        log.info(`${lhd} << complete list all device. count [${itemCount + itemCount2}], offset [${offset}], limit [${limit}]`);
        
        // 스트리밍 방식이므로 return하지 않음 (이미 res.end() 호출됨)
    } catch (error) {
        log.error(`${lhd} << failed list all device. error [${error.message}]`);

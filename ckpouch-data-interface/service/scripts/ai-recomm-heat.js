...
module.exports = async (ctx, src, packet, listener) => {
    ...
    if (!validatePacketInfo.succ) {
        log.warn(`${lhd} << failed control device by tp.${validatePacketInfo.data.message ? ` ${validatePacketInfo.data.message}` : ''}`);
        return sUtils.makeCkpush4Response(sValues.ERROR_CODE.WRONG_REQUEST, 'Wrong request', packet.hd.tid);
    }
    ...
    const selectAnalysisInfo = await utils.postgresql.select('analysis', null, 'tbl_heat_forecast', null, selectAnalysisFilter, selectAnalysisSort, null, 0, 1, lhd);
    if (!selectAnalysisInfo.succ) {
        log.error(`${lhd} << failed list device. failed select device sync his. err [${selectAnalysisInfo.err}]`);
    ...
    return {
        ...
        list: selectAnalysisInfo.data[0].temperature_data,
    };
};
